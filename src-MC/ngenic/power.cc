...
// NEW: Growth function for AXION DARK MATTER in linear approximation. D=1+3x/2, x=a/a_eq
double ngenic::ngenic_growth(double a)
{
  double aeq = 1./(All.zeq + 1.);
  double result;
  result = 1. + 3./2. * a/aeq;
  return result;
}
...
//NEW: Function f=dlogD/dloga for AXION DARK MATTER
double ngenic::ngenic_f1_omega(double a)
{
  double aeq = 1./(All.zeq + 1.);
  double result;
  result = 3./2. * a/aeq * 1/(1+3./2. * a/aeq);
  return result;
}
